...
pub struct ClaimevenRule;

impl RuleGenerator for ClaimevenRule {
    ...
    fn generate(&self, position: &Position, facts: &PositionFacts) -> Vec<Solution> {
        ...
        for col in 0..crate::board::COLUMNS {
            ...
            for lower_row in (1..crate::board::ROWS).step_by(2) {
                ...
                let solves_groups =
                    problem_groups_solved_by_cells(facts, |group| group.cells.contains(&upper));
                ...
                solutions.push(Solution {
                    ...
                    solves_groups,
                    explanation: format!("Claimeven {}{}", column_name(col), upper.row),
                });
            }
        }
